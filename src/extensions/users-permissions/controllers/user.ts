...
export default {
  async sendPasswordEmail(ctx: any) {
    try {
      const { id } = ctx.params;
      ...
      // Reutilizar la función helper del index.ts
      // Nota: Necesitamos acceder a la función desde el contexto global
      // Por ahora, usaremos la implementación inline pero simplificada
